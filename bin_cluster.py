# Program 2: bin_cluster.py : Cluster the bins
# Author: Pratik Mishra, Iowa State University.
# Compile : python bin_cluster.py trace_file_from_system_extracted_info.csv
# The input file is the output from extract_info.py
# Output:trace_file_from_system_extracted_info.csv_bin_info.csv
# Format:bin_number,bin_count,xfrlen_bin_sum,end_time
...
def read_extracted_csv():
        ...
        for y in clustered_bins:
                ...
                xfrlen_bin_sum.append(sum(xfrlen_list[flag_counter:counter])/(2*1024.0)) # 512/1024=1/2
                flag_counter = flag_counter + len(y)                
                        
        single_bin = []
        single_bin = zip(bin_number,bin_count,xfrlen_bin_sum,operation_bin,end_time)
        #sorted(zip(bin_number,bin_count,xfrlen_bin_sum,operation_bin,end_time),key=itemgetter(2),reverse=True)
